...
namespace JPS
{
  ...
  class GraphSearch
  {
    ...
    //   GraphSearch(const char* cMap, int xDim, int yDim, int zDim, double eps = 1, bool verbose = false);
      ...
      GraphSearch(nmoma_planner::GridMap::Ptr Map, const double &safe_dis);
      static std::vector<Eigen::Vector4d> getDensePath(const std::vector<Eigen::Vector2d>& raw_path,
      ...
      std::vector<Eigen::Vector2d> plan2dJPS(const Eigen::Vector2d& start, 
      ...
      typedef shared_ptr<GraphSearch> Ptr;
    ...
    private:
      ...
      // const char* cMap_;
      nmoma_planner::GridMap::Ptr map_;
      ...
      static void normalizeAngle(const double &ref_angle, double &angle);
 };
}
#endif
