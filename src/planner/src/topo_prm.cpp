...
void TopologyPRM::init(ros::NodeHandle& nh) {
  ...
  resolution_ = grid_map_ptr->getResolution();
  offset_ = grid_map_ptr->getOffset();
...
list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start, Eigen::Vector3d end) {
  ...
  while (sample_time < max_sample_time_ && sample_num < max_sample_num_) {
    ...
    double dist;
    //! xulong for 2D
    // grid_map_ptr->getDistance2d(pt.head(2), dist);
    // dist -= 0.4;
    dist = grid_map_ptr->getDistCoarse2d(pt.head(2), use_critical);
    if (dist <= clearance_) {
...
vector<GraphNode::Ptr> TopologyPRM::findVisibGuard(Eigen::Vector3d pt) {
  ...
  for (list<GraphNode::Ptr>::iterator iter = graph_.begin(); iter != graph_.end(); ++iter) {
    ...
    //! xulong for safe
    // if (lineVisib(pt, (*iter)->pos_, clearance_, pc)) {
...
bool TopologyPRM::lineVisib(const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, double thresh,
  ...
  // std::cout  <<  "setInput: " << p1.transpose() << " " << p2.transpose() << std::endl;
  Eigen::Vector3d start = p1 / resolution_;
  Eigen::Vector3d end = p2 / resolution_;
  bool flag = casters_[caster_id].setInput(p1 / resolution_, p2 / resolution_);
  Eigen::Vector3i prev;
  prev(0) = (int)std::floor(start(0));
  prev(1) = (int)std::floor(start(1));
  prev(2) = (int)std::floor(start(2));
  while (flag && casters_[caster_id].step(ray_pt)) {
    ...
    dist = grid_map_ptr->getDistCoarse2i(pt_id.head(2), use_critical);
    // grid_map_ptr->getDistance2d(ray_pt.head(2) * resolution_, dist);
    // dist -= 0.4;
    if (dist <= thresh) {
      Eigen::Vector3d pc1, pc2;
      grid_map_ptr->indexToPos3d(pt_id, pc1);
      grid_map_ptr->indexToPos3d(prev, pc2);
      pc = 0.5 * (pc1 + pc2);
      pc(2) = 0.0;
      // grid_map_ptr->indexToPos3d(pt_id, pc);
      return false;
    }
    prev = pt_id;
  }
...
bool TopologyPRM::sameTopoPath(const vector<Eigen::Vector3d>& path1,
  ...
  for (int i = 0; i < pt_num; ++i) {
    //! xulong for safe
    // if (!lineVisib(pts1[i], pts2[i], clearance_, pc)) {
...
void TopologyPRM::shortcutPath(vector<Eigen::Vector3d> path, int path_id, int iter_num) {
  ...
  for (int k = 0; k < iter_num; ++k) {
    ...
    for (int i = 1; i < dis_path.size(); ++i) {
      //! xulong for safe
      // if (lineVisib(short_path.back(), dis_path[i], clearance_, colli_pt, path_id)) continue;
      if (lineVisib(short_path.back(), dis_path[i], resolution_, colli_pt, path_id)) continue;

      //! xulong for 2D
      // grid_map_ptr->getDisWithGradI3d(colli_pt, dist, grad);
      Eigen::Vector2d grad_2d;
      // grid_map_ptr->getDisWithGradI2d(colli_pt.head(2), dist, grad_2d);
      grid_map_ptr->getDisWithGradI2d(colli_pt.head(2), dist, grad_2d, false, use_critical);
      // grid_map_ptr->getDisWithGradI2d(colli_pt.head(2), dist, grad_2d, true, use_critical);
...
void TopologyPRM::setEnv(const GridMap::Ptr& env) { this->grid_map_ptr = env; }
