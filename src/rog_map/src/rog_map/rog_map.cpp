...
ROGMap::ROGMap(const ros::NodeHandle& nh) :nh_(nh) {
    ...
    if (cfg_.visualization_en) {
        ...
        if (cfg_.esdf_en) {
            ...
            vm_.esdf_pub3d = nh_.advertise<sensor_msgs::PointCloud2>("rog_map/esdf3d", 1);
            vm_.esdf_neg_pub = nh_.advertise<sensor_msgs::PointCloud2>("rog_map/esdf/neg", 1);
            vm_.esdf_neg_pub3d = nh_.advertise<sensor_msgs::PointCloud2>("rog_map/esdf3d/neg", 1);
            vm_.esdf_occ_pub = nh_.advertise<sensor_msgs::PointCloud2>("rog_map/esdf/occ", 1);

            vm_.critical_esdf_pub = nh_.advertise<sensor_msgs::PointCloud2>("rog_map/esdf/critical", 1);
            vm_.critical_esdf_grad_pub = nh_.advertise<visualization_msgs::MarkerArray>("rog_map/esdf/critical_grad", 1);

            vm_.flat_esdf_pub = nh_.advertise<sensor_msgs::PointCloud2>("rog_map/esdf/flat", 1);
        }
...
void ROGMap::vizCallback(const ros::TimerEvent& event) {
    ...
    if (vm_.occ_pub.getNumSubscribers() >= 1) {
        ...
        //! xulong to public_occ
        Vec3f esdf_box_max, esdf_box_min;
        esdf_map_->getUpdatedBbox(esdf_box_min, esdf_box_max);
        if ((esdf_box_max - esdf_box_min).minCoeff() > 0)
        {
            boxSearch(esdf_box_min, esdf_box_max, OCCUPIED, occ_map);
            vecEVec3fToPC2(occ_map, cloud_msg);
            public_occ = cloud_msg;
            vm_.occ_pub.publish(cloud_msg);
        }
    }
    ...
    if (cfg_.esdf_en) {
        ...
        if (vm_.esdf_pub3d.getNumSubscribers() >= 1) {
            esdf_map_->getPositiveESDFPC2(box_min, box_max, cloud_msg);
            cloud_msg.header.stamp = ros::Time::now();
            //! xulong add public_sdf
            public_sdf = cloud_msg;
            vm_.esdf_pub3d.publish(cloud_msg);
        }

        if (vm_.esdf_neg_pub3d.getNumSubscribers() >= 1) {
            esdf_map_->getNegativeESDFPC2(box_min, box_max, robot_state_.p.z() - 0.5, cloud_msg);
            cloud_msg.header.stamp = ros::Time::now();
            vm_.esdf_neg_pub3d.publish(cloud_msg);
        }

        if(vm_.critical_esdf_pub.getNumSubscribers() >= 1) {
            esdf_map_->getCriticalESDFPC2(box_min, box_max, robot_state_.p.z() - 0.5, cloud_msg);
            cloud_msg.header.stamp = ros::Time::now();
            vm_.critical_esdf_pub.publish(cloud_msg);
        }

        if(vm_.critical_esdf_grad_pub.getNumSubscribers() >= 1) {
            visualization_msgs::MarkerArray marker_array;
            esdf_map_->getESDFGradMarker(box_min, box_max, robot_state_.p.z() - 0.5, marker_array);
            for(int i = 0; i < marker_array.markers.size(); i++)
            {
                marker_array.markers[i].header.stamp = ros::Time::now();
            }
            vm_.critical_esdf_grad_pub.publish(marker_array);
        }

        if(vm_.flat_esdf_pub.getNumSubscribers() >= 1) {
            esdf_map_->getFlatESDFPC2(box_min, box_max, cloud_msg);
            cloud_msg.header.stamp = ros::Time::now();
            vm_.flat_esdf_pub.publish(cloud_msg);
        }
