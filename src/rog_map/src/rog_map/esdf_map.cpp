...
namespace rog_map {
    ...
    void ESDFMap::initESDFMap(const rog_map::Vec3i &half_prob_map_size_i, const double &prob_map_resolution,
        ...
        distance_buffer_2d.resize(sc_.map_size_i(0) * sc_.map_size_i(1));
        distance_buffer_flat.resize(sc_.map_size_i(0) * sc_.map_size_i(1));
    ...
    void ESDFMap::printPosInfo(const Vec3f &pos) {
        Vec3i id_g, id_l;
        posToGlobalIndex(pos, id_g);
        globalIndexToLocalIndex(id_g, id_l);

        std::cout << "id_g: " << id_g.transpose() << std::endl;
        std::cout << "id_l: " << id_l.transpose() << std::endl;
    }
    ...
    double ESDFMap::getCriticalDistance(const rog_map::Vec3f &pos) const {
        Vec3i id_g, id_l;
        posToGlobalIndex(pos, id_g);
        globalIndexToLocalIndex(id_g, id_l);
        id_l += sc_.half_map_size_i;
        int id = id_l[0] * sc_.map_size_i(1) + id_l[1];
        return distance_buffer_2d[id];
    }

    double ESDFMap::getCriticalDistance(const Vec3i &id_g) const {
        Vec3i id_l;
        globalIndexToLocalIndex(id_g, id_l);
        id_l += sc_.half_map_size_i;
        int id = id_l[0] * sc_.map_size_i(1) + id_l[1];
    ...
    double ESDFMap::getDistance2d(const rog_map::Vec3f &pos) const {
        Vec3i id_g, id_l;
        posToGlobalIndex(pos, id_g);
        globalIndexToLocalIndex(id_g, id_l);
        id_l += sc_.half_map_size_i;
        int id = id_l[0] * sc_.map_size_i(1) + id_l[1];
        return distance_buffer_flat[id];
    }

    double ESDFMap::getDistance2d(const Vec3i &id_g) const {
        Vec3i id_l;
        globalIndexToLocalIndex(id_g, id_l);
        id_l += sc_.half_map_size_i;
        int id = id_l[0] * sc_.map_size_i(1) + id_l[1];
    ...
    bool ESDFMap::isLineFree2d(const Eigen::Vector2d &start, const Eigen::Vector2d &end, double threshold) const {
        Vec3f start_pos(start(0), start(1), 0);
        Vec3f end_pos(end(0), end(1), 0);
        
        Vec3i start_id_g, start_id_l;
        posToGlobalIndex(start_pos, start_id_g);
        globalIndexToLocalIndex(start_id_g, start_id_l);
        start_id_l += sc_.half_map_size_i;

        Vec3i end_id_g, end_id_l;
        posToGlobalIndex(end_pos, end_id_g);
        globalIndexToLocalIndex(end_id_g, end_id_l);
        end_id_l += sc_.half_map_size_i;
        
        raycaster::RayCaster raycaster;
        raycaster.setResolution(sc_.resolution);
        
        Vec3f ray_pt;
        raycaster.setInput(start_pos, end_pos);
        while (raycaster.step(ray_pt)) {
            Vec3f pos3d = ray_pt;
            Vec3i id_g, id_l;
            posToGlobalIndex(pos3d, id_g);
            globalIndexToLocalIndex(id_g, id_l);
            id_l += sc_.half_map_size_i;
            int id = id_l[0] * sc_.map_size_i(1) + id_l[1];
            if (distance_buffer_flat[id] < threshold)
                return false;
        }
        return true;
    }
    ...
    void ESDFMap::updateESDF3D(const rog_map::Vec3f &cur_odom) {
        // std::cout<<RED<<" -- [ESDFMap] Update ESDF map."<<RESET<<std::endl;
        ...
        auto getEsdfLocalIndexHash2D=[&](const int &id_x,
                                         const int &id_y) {
            return id_x * sc_.map_size_i(1) + id_y;
        };
        ...
        true;
        {
            std::vector<double> neg_buffer, tmp_buffer;
            neg_buffer.resize(sc_.map_size_i(0) * sc_.map_size_i(1));
            tmp_buffer.resize(sc_.map_size_i(0) * sc_.map_size_i(1));
            std::vector<bool> occ_buffer;
            occ_buffer.resize(sc_.map_size_i(0) * sc_.map_size_i(1), false);

            // std::cout << " -- [ESDFMap] Compute 2D critical map." << std::endl;
            // fill in 2d occ buffer
            for(int x = min_esdf(0); x <= max_esdf(0); ++x)
            for(int y = min_esdf(1); y <= max_esdf(1); ++y)
            for(int z = min_esdf(2); z <= max_esdf(2); ++z) {
                int _x, _y, _z;
                _x = x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0];
                _y = y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1];
                _z = z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2];
                bool occ = isOccupied(getEsdfLocalIndexHash(_x, _y, _z));
                occ_buffer[getEsdfLocalIndexHash2D(_x, _y)] = occ;
                // distance_buffer_2d[getEsdfLocalIndexHash2D(_x, _y)] = occ ? 0 : 1;
                if (occ) break;
            }

            /* ========== compute positive DT ========== */
            // std::cout << " -- [ESDFMap] Compute positive DT." << std::endl;
            for(int x = min_esdf(0); x <= max_esdf(0); ++x) {
                int _x = x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0];
                fillESDF(
                    [&](int y) {
                        return occ_buffer[getEsdfLocalIndexHash2D(_x, y)] ? 
                        0 : std::numeric_limits<double>::max();
                    },
                    [&](int y, double val) { tmp_buffer[getEsdfLocalIndexHash2D(_x, y)] = val; },
                    min_esdf(1), max_esdf(1), 1, id_l(1)
                );
            }

            for(int y = min_esdf(1); y <= max_esdf(1); ++y) {
                int _y = y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1];
                fillESDF(
                    [&](int x) {
                        return tmp_buffer[getEsdfLocalIndexHash2D(x, _y)];
                    },
                    [&](int x, double val) { 
                        distance_buffer_2d[getEsdfLocalIndexHash2D(x, _y)] = sc_.resolution * std::sqrt(val); },
                    min_esdf(0), max_esdf(0), 0, id_l(0)
                );
            }
            ...
            for(int y = min_esdf(1); y <= max_esdf(1); ++y) {
                ...
                fillESDF(
                    ...
                    [&](int x, double val) { 
                        neg_buffer[getEsdfLocalIndexHash2D(x, _y)] = sc_.resolution * std::sqrt(val); },
                    min_esdf(0), max_esdf(0), 0, id_l(0)
                );
            }
            
            // /* ========== combine pos and neg distance ========== */
            for(int x = min_esdf(0); x <= max_esdf(0); ++x)
            for(int y = min_esdf(0); y <= max_esdf(0); ++y) {
                // int _x = x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0];
                // int _y = y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1];
                int idx = getEsdfLocalIndexHash2D(x, y);
                distance_buffer_2d[idx] = neg_buffer[idx] > 0.0 ? 
                    distance_buffer_2d[idx] - neg_buffer[idx] + sc_.resolution : distance_buffer_2d[idx];
            }
        }
        ...
        true;
        {
            ...
            int _height_l;
            {
                Vec3f _pos(0, 0, 0.155);
                Vec3i _g, _l;
                posToGlobalIndex(_pos, _g);
                globalIndexToLocalIndex(_g, _l);
                _l += sc_.half_map_size_i;
                _height_l = _l(2) < max_esdf(2) ? _l(2) : max_esdf(2);
            }
            for(int x = min_esdf(0); x <= max_esdf(0); ++x)
            for(int y = min_esdf(1); y <= max_esdf(1); ++y)
            for(int z = min_esdf(2); z <= _height_l; ++z) {
                int _x, _y, _z;
                _x = x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0];
                _y = y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1];
                _z = z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2];
            ...
            for(int y = min_esdf(1); y <= max_esdf(1); ++y) {
                ...
                fillESDF(
                    ...
                    [&](int x, double val) { 
                        distance_buffer_flat[getEsdfLocalIndexHash2D(x, _y)] = sc_.resolution * std::sqrt(val); },
                    min_esdf(0), max_esdf(0), 0, id_l(0)
                );
            }
            ...
            for(int y = min_esdf(0); y <= max_esdf(0); ++y) {
                ...
                // int _y = y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1];
                int idx = getEsdfLocalIndexHash2D(x, y);
                distance_buffer_flat[idx] = neg_buffer[idx] > 0.0 ? 
                    distance_buffer_flat[idx] - neg_buffer[idx] + sc_.resolution : distance_buffer_flat[idx];
            }
        } // end of distance_buffer_flat

        // for (int x = min_esdf(0); x <= max_esdf(0); ++x)
        // for (int y = min_esdf(1); y <= max_esdf(1); ++y){
        //     int _x = x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0];
        //     int _y = y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1];
        //     double min_distance = 10e3;
        //     for (int z = min_esdf(2); z <= max_esdf(2); ++z){
        //         int _z = z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2];
        //         Vec3i id_g;
        //         Vec3i id_l = Vec3i(x, y, z) - sc_.half_map_size_i;
        //         localIndexToGlobalIndex(id_l, id_g);
        //         if(id_g(2) <= update_local_map_min_i_(2) || 
        //            id_g(2) >= update_local_map_max_i_(2)) continue;
        //         double dist = distance_buffer[getEsdfLocalIndexHash(_x, _y, _z)];
        //         min_distance = dist < min_distance ? dist : min_distance;
        //     }
        //     distance_buffer_2d[getEsdfLocalIndexHash2D(_x, _y)] = min_distance;
        // }
...
//            box_max_i = box_max_i.cwiseMin(update_local_map_max_i_);
    ...
    void ESDFMap::getCriticalESDFPC2(const rog_map::Vec3f &box_min_d, const rog_map::Vec3f &box_max_d,
                                     const double &visualize_z, sensor_msgs::PointCloud2 &pc2) {
        std::lock_guard<std::mutex> lck(update_esdf_mtx);
        pcl_pc.clear();
        Vec3i box_min_i, box_max_i;
        ...
        for (int x = box_min_i.x(); x <= box_max_i.x(); x++) {
            for (int y = box_min_i.y(); y <= box_max_i.y(); y++) {
                Vec3i id_g(x, y, z);
                double dist = getCriticalDistance(id_g);
                Vec3f pos;
                globalIndexToPos(id_g, pos);
                pcl::PointXYZI pt;
                pt.x = pos(0);
    ...
    void ESDFMap::getFlatESDFPC2(const rog_map::Vec3f &box_min_d, const rog_map::Vec3f &box_max_d,
        sensor_msgs::PointCloud2 &pc2) {
    ...
    void ESDFMap::getESDFGradMarker(const Vec3f &box_min_d,
        ...
        for (int y = box_min_i.y(); y <= box_max_i.y(); y++) {
            Vec3i id_g(x, y, z);
            Vec3f pos3d, grad3d;
            globalIndexToPos(id_g, pos3d);
            {
                double dist;
                getValueGrad2d(pos3d, dist, grad3d);
            }
            grad3d(2) = 0.0;
            grad3d.normalize();
            // Vec3f rotaxis = Vec3f::UnitZ().cross(grad3d).normalized();
            // double angle = acos(Vec3f::UnitZ().dot(grad3d));
            Eigen::Quaterniond quat = Eigen::Quaterniond::FromTwoVectors(Vec3f::UnitX(), grad3d);
            quat.normalize();
            ...
            marker.pose.position.x = pos3d(0);
            marker.pose.position.y = pos3d(1);
            marker.pose.position.z = pos3d(2);
    ...
    // EDT Environment
    void ESDFMap::getSurroundPts(const Vec3f& pos, Vec3f pts[2][2][2], Vec3f & diff) const
    {
    ...
    void ESDFMap::getSurroundDistance(Eigen::Vector3d pts[2][2][2], double dists[2][2][2]) const
    {
        for (int x = 0; x < 2; x++) {
            for (int y = 0; y < 2; y++) {
                for (int z = 0; z < 2; z++) {
    ...
    void ESDFMap::getValueGrad(const Eigen::Vector3d& pos, double& dist, Eigen::Vector3d& grad) const
    {
        Eigen::Vector3d diff;
        Eigen::Vector3d sur_pts[2][2][2];
        getSurroundPts(pos, sur_pts, diff);
        ...
        double v00 = (1 - diff(0)) * dists[0][0][0] + diff(0) * dists[1][0][0]; // b
        double v01 = (1 - diff(0)) * dists[0][0][1] + diff(0) * dists[1][0][1]; // d
        double v10 = (1 - diff(0)) * dists[0][1][0] + diff(0) * dists[1][1][0]; // a
        double v11 = (1 - diff(0)) * dists[0][1][1] + diff(0) * dists[1][1][1]; // c
    ...
    void ESDFMap::getCriticalValueGrad(const Eigen::Vector3d& pos, double& dist, Eigen::Vector3d& grad) const
    {
        Eigen::Vector3d diff;
        Eigen::Vector3d sur_pts[2][2];
        {   // get 2d surrounding points
            Vec3f mask(1.0, 1.0, 0.0); // ignore z-axis
            Vec3f pos_m = pos - 0.5 * sc_.resolution * mask;
            Vec3i idx;
            Vec3f idx_pos;
        ...
        double dists[2][2];
        {
            for (int x = 0; x < 2; x++)
            for (int y = 0; y < 2; y++)
                dists[x][y] = getCriticalDistance(sur_pts[x][y]);
        }

        
        double fxy1 = diff(0) * dists[1][0] + (1 - diff(0)) * dists[0][0];
        double fxy2 = diff(0) * dists[1][1] + (1 - diff(0)) * dists[0][1];
        dist = (1 - diff(1)) * fxy1 + diff(1) * fxy2;
        
        grad.setZero();
        double dfxy1_dd0 = dists[1][0] - dists[0][0];
        double dfxy2_dd0 = dists[1][1] - dists[0][1];
        grad(0) = (1 - diff(1)) * dfxy1_dd0 + diff(1) * dfxy2_dd0;
        grad(1) = -fxy1 + fxy2;
        grad *= sc_.resolution_inv;
        
        // if (pos(0) == 0 && pos(1) == 0 && pos(2) == 0) {
        //     std::cout << "dists: " << dists[0][0] << " " << dists[0][1] << " " << dists[1][0] << " " << dists[1][1] << std::endl;
        //     std::cout << "dist: " << dist << std::endl;
        // }
        return;
    }

    void ESDFMap::getValueGrad2d(const Eigen::Vector3d& pos, double& dist, Eigen::Vector3d& grad) const
    {
        Eigen::Vector3d diff;
        Eigen::Vector3d sur_pts[2][2];
        {   // get 2d surrounding points
        ...
        double dists[2][2];
        {
            for (int x = 0; x < 2; x++)
            for (int y = 0; y < 2; y++)
                dists[x][y] = getDistance2d(sur_pts[x][y]); //Fix this
        }
