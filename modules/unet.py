...
class UNetModel(nn.Module):
    def __init__(self, cfg: DictConfig, slurm: bool, *args, **kwargs) -> None:
        ...
        if cfg.data_normer == "shoot" and cfg.scene_model.fusion_method == "pspace":
            self.spacer = SPACER["shoot_pspace"]

        ## create scene model from config
        self.cond_backbone = cfg.scene_model.cond_backbone
        self.fusion_method = cfg.scene_model.fusion_method
        ...
        if self.cond_backbone == "simple":
            ...
            if self.fusion_method == "proposed":
                cfg.scene_model.c = 3
                cfg.scene_model.ptmamba.c = 3
                blocks = [4, 16, 64, 256]
                layers =[]
                for i in range(len(blocks) - 1):
                    layers.append(nn.Linear(blocks[i], blocks[i+1]))
            ...
            elif self.fusion_method == "pspace":
                cfg.scene_model.c = 8
                cfg.scene_model.ptmamba.c = 8
            elif self.fusion_method == "attention":
                cfg.scene_model.c = 3
                cfg.scene_model.ptmamba.c = 3
                sg_head = 18
                self.sg_inlayer = nn.Linear(self.d_sgin, 16*sg_head, bias=False)
                blocks = [16, 64, 256]
                layers =[]
                for i in range(len(blocks) - 1):
                    layers.append(nn.Linear(blocks[i], blocks[i+1]))
                ...
                layers.append(nn.Linear(blocks[-1], self.d_model))
                layers.append(nn.LayerNorm(self.d_model))
                self.sg_encoder1 = nn.Sequential(*layers)
                sg_head = 18
                self.sg_inlayer = nn.Linear(self.d_sgin, 16*sg_head, bias=False)
                blocks2 = [16, 64, 256]
                layers2 =[]
                for i in range(len(blocks2) - 1):
                    layers2.append(nn.Linear(blocks2[i], blocks2[i+1]))
                    layers2.append(nn.LayerNorm(blocks2[i+1]))
                    layers2.append(nn.SiLU())
                layers2.append(nn.Linear(blocks2[-1], self.d_model))
                layers2.append(nn.LayerNorm(self.d_model))
                self.sg_encoder2 = nn.Sequential(*layers2)

                layers3 = [\
                    nn.Linear(self.d_model * 2, self.d_model),\
                    nn.LayerNorm(self.d_model)
                ]
                self.sg_fuse_in= nn.Sequential(*layers3)
                ...
                raise ValueError("Invalid fusion_method")
            ...
            if self.cond_backbone == "minkresnet":
                ...
                #                         stride=(2, 4, 2),
                #                         enc_depths=(2, 2, 4, 2),
                #                         enc_channels=(32, 128, 256, 512),
                #                         enc_num_head=(2, 8, 16, 32),
    ...
    def forward(self, x_t: torch.Tensor, ts: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        ...
        ## time embedding
        t_emb = timestep_embedding(ts, self.d_model, x_t)
        ...
        if self.use_position_embedding:
            ...
            pos_embedding_Q = timestep_embedding(pos_Q, DX, x_t) # <L, d_model>
    ...
    def condition(self, data: Dict) -> torch.Tensor:
        ...
        if self.cond_backbone == "simple":
            ...
            env_in = pos.clone()
            fusion_method = "proposed" # "pspace" or "attention"
            if self.fusion_method == "proposed":
                psg = self.sg_encoder(self.spacer(startgoals).half())
                psg = rearrange(psg, 'b l c -> b c l')
                pos_Q = torch.arange(psg.shape[-1], dtype=psg.dtype, device=psg.device)
                pos_embedding_Q = timestep_embedding(pos_Q, psg.shape[-2], psg) # <L, d_model>
                psg = psg + pos_embedding_Q.permute(1, 0) # <B, d_model, L>
                psg = rearrange(psg, 'b c l -> b l c')
            elif self.fusion_method == "pspace":
                feat_occ = torch.Tensor([100.0, 100.0, 0., 0., 1.]).cuda().repeat(b, env_in.shape[1], 1).half()
                env_in = torch.cat([env_in, feat_occ], dim=2)
                psg = self.spacer(startgoals).half()
                env_in = torch.cat([psg, env_in], dim=1)
            elif self.fusion_method == "attention":
                sg_seq = self.sg_inlayer(startgoals)
                sg_seq = rearrange(sg_seq, 'b (s d) -> b s d', s=18)
                psg = self.sg_encoder(sg_seq)
            elif self.fusion_method == "fuse":
                # attention
                sg_seq = self.sg_inlayer(startgoals)
                sg_seq = rearrange(sg_seq, 'b (s d) -> b s d', s=18)
                psg1 = self.sg_encoder2(sg_seq)
                # spacer
                psg = self.sg_encoder1(self.spacer(startgoals).half())
                psg = rearrange(psg, 'b l c -> b c l')
                pos_Q = torch.arange(psg.shape[-1], dtype=psg.dtype, device=psg.device)
                pos_embedding_Q = timestep_embedding(pos_Q, psg.shape[-2], psg) # <L, d_model>
                psg = psg + pos_embedding_Q.permute(1, 0) # <B, d_model, L>
                psg = rearrange(psg, 'b c l -> b l c')
                # psg = 0.0 * psg + 1.0 * psg1
                psg = torch.concat([psg, psg1], dim=2)

                psg = self.sg_fuse_in(psg)
            elif self.fusion_method == "none":
                pass
            else:
                raise ValueError("Invalid fusion_method")
            ...
            if self.cond_backbone == "minkresnet":
                ...
                cond_feat = sampled_features + pos_feats
                cond_mask = (sampled_coords[:, :, -1] == -1)
            elif self.cond_backbone == "ptans3":
                offset = torch.ones(b, dtype=torch.int).cuda() * env_in.shape[1]
                for i in range(b):
                    offset[i] *= (i+1)
                points_in = {'coord': rearrange(env_in[..., :3], 'b n c -> (b n) c'),
                ...
                poses = []
                feats = []
                max_len = max(points_out['offset'].diff().max(), points_out['offset'][0]).item() if offset.shape[0] > 1 else points_out['offset'][0].item()
                for i in range(b):
                    if i==0:
                        begin = 0
                    else:
                ...
                cond_feat = feats + pos_feats
                cond_mask = (poses[:, :, -1] == -1)
            elif self.cond_backbone == "ptmamba":
                pts, feats = self.scene_model(env_in[..., :3].contiguous(), env_in.contiguous())
                pos_feats = self.pos_embed_env(pts*self.voxel_size).transpose(1, 2).contiguous()
                cond_feat = feats + pos_feats
                cond_mask = None
            
            if self.fusion_method == "proposed":
                cond_feat = self.sg_fusion(psg, cond_feat, occ_mask=cond_mask)
            elif self.fusion_method == "pspace":
                pass
            elif self.fusion_method == "attention":
                cond_feat = self.sg_fusion(psg, cond_feat, occ_mask=cond_mask)
