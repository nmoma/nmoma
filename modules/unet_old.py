...
from utils.utils import SPACER, timestep_embedding, filter,ResBlock, SpatialTransformer, BasicTransformerBlock, PositionEmbeddingLearned
...
class UNetModel(nn.Module):
    def __init__(self, cfg: DictConfig, slurm: bool, *args, **kwargs) -> None:
        ...
        if self.cond_backbone == "simple":
            ...
            self.voxel_size = 0.1
            if self.add_spacer:
                cfg.scene_model.num_points += 18
                cfg.scene_model.c = 8
            self.scene_model = MinkResNet(depth=34, in_channels=cfg.scene_model.c, max_channels=self.d_model)
            self.pos_embed_env = PositionEmbeddingLearned(3, self.context_dim)
            self.sg_fusion = BasicTransformerBlock(dim=self.context_dim, n_heads=2, d_head=128, 
                                                   dropout=0.0, context_dim=self.d_startgoal, 
                                                   gated_ff=True, mult_ff=2)
        ...
        # self.cls_pool = nn.AdaptiveAvgPool1d(1)  # 全局平均池化
    ...
    def forward(self, x_t: torch.Tensor, ts: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        ...
        #! xulong: check if inference work
        # h = filter(h)
    ...
    def condition(self, data: Dict) -> torch.Tensor:
        ...
        if self.cond_backbone == "simple":
            ...
            env_in = pos.clone()
            if self.add_spacer:
                feat_occ = torch.Tensor([100.0, 100.0, 0., 0., 1.]).cuda().repeat(b, env_in.shape[1], 1)
                env_in = torch.cat([env_in, feat_occ], dim=2)
                psg = self.spacer(startgoals)
            ...
            if self.start_goal_enc is not None:
                startgoal_emd = self.start_goal_enc(startgoals).unsqueeze(1)
                startgoal_emd_full = startgoal_emd.repeat(1, sampled_features.shape[1], 1)
                sampled_features = torch.cat([sampled_features, startgoal_emd_full], dim=2)
                #! xulong: try use othor PE, such as xCPE, CPE, APE, etc.
                ...
                sampled_features = self.sg_fusion(sampled_features, \
                                        context=startgoal_emd, mask=(sampled_coords[:, :, -1] == -1))
            cond_feat = sampled_features
        elif self.cond_backbone == "ptans3":
            pos, startgoals = data['pos'], data['startgoal']
            env_in = pos.clone()
            if self.add_spacer:
            ...
            if self.start_goal_enc is not None:
                startgoal_emd = self.start_goal_enc(startgoals).unsqueeze(1)
                startgoal_emd_full = startgoal_emd.repeat(1, feats.shape[1], 1)
                feats = torch.cat([feats, startgoal_emd_full], dim=2)
                #! xulong: try use othor PE, such as xCPE, CPE, APE, etc.
                pos_feats = self.pos_embed_env(poses*self.voxel_size).transpose(1, 2).contiguous()
                feats = feats + pos_feats
                feats = self.sg_fusion(feats, context=startgoal_emd, mask=(poses[..., -1] == -1))
            cond_feat = feats
        elif self.cond_backbone == "ptmamba":
            pos, startgoals = data['pos'], data['startgoal']
            env_in = pos.clone()
            if self.add_spacer:
                ...
                env_in = torch.cat([psg, env_in], dim=1)
                pos = torch.cat([psg[..., :3], pos], dim=1)
            pts, feats = self.scene_model(pos, env_in)
            if self.start_goal_enc is not None:
                startgoal_emd = self.start_goal_enc(startgoals).unsqueeze(1)
                startgoal_emd_full = startgoal_emd.repeat(1, feats.shape[1], 1)
                feats = torch.cat([feats, startgoal_emd_full], dim=2)
                ...
                feats = feats + pos_feats
                feats = self.sg_fusion(feats, context=startgoal_emd, mask=None)
            cond_feat = feats
