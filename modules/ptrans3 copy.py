...
class Ptrans3(PointModule):
    ...
    def forward(self, data_dict, tdict = {}):
        """
        ...
        point.sparsify()
        torch.cuda.synchronize()
        tdict['sparsify'] = time.time() - start_time

        start_time = time.time()
        point = self.embedding(point)
        torch.cuda.synchronize()
        tdict['embedding'] = time.time() - start_time
        ...
        torch.cuda.synchronize()
        tdict['decoder'] = time.time() - start_time
        return point
...
if __name__ == "__main__":
    ...
    '''
    feat_dim = 10
    model = Ptrans3(in_channels=feat_dim).cuda()
    batch_size = 16
    point_num = 1024
    pos = torch.rand(point_num * batch_size, 3).cuda() * 10.0
    feat = torch.rand(point_num * batch_size, feat_dim).cuda()
    offset = torch.ones(batch_size, dtype=torch.int).cuda() * point_num
    ...
    cond_data = {'coord': pos,
                ...
                'feat': feat,
                'offset': offset}
    cond = model(cond_data)
    print(cond.keys())
    print(cond["offset"])
    a = offset2batch(cond["offset"])
    print(a.shape)
    print(cond["feat"].shape)
    ...
    print(f'total model size is {sum(nparams)}.')