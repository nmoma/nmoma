...
class MinkResNet(nn.Module):
    ...
    def forward(self, x, tdict={}):
        ...
        x = self.conv1(x)
        torch.cuda.synchronize()
        tdict['conv1'] = time.time() - start_time
        ...
        for i in range(self.num_stages):
            ...
            # outs.append(x)
        torch.cuda.synchronize()
        tdict['layers'] = time.time() - start_time
        return x
        # return outs
        
if __name__ == '__main__':
    in_channels = 8
    # model = MinkResNet(depth=34, in_channels=in_channels, max_channels=512).cuda()
    model = MinkResNet(depth=18, in_channels=in_channels, max_channels=256).cuda()
    params = []
    nparams = []
    ...
    start_time = time.time()
    voxel_size = 0.01
    points = torch.rand(8, 10000, in_channels).cuda()*10.0
    coordinates, features = ME.utils.batch_sparse_collate(
                [(p[:, :3] / voxel_size, p[:, 0:] if p.shape[1] > 3 else p[:, :3]) for p in points],
