...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz, feats, tdict ={}):
        '''
            input: B N 3; B N C
            ---------------------------
            output: B G M C
        ...
        torch.cuda.synchronize()
        tdict['fps'] = time.time() - start_time
        ...
        torch.cuda.synchronize()
        tdict['knn'] = time.time() - start_time
...
def serialization(pos, feat=None, x_res=None, order="z", layers_outputs=[], grid_size=0.02, tdict={}):
    bs, n_p, _ = pos.size()
    ...
    grid_coord = torch.ones_like(scaled_coord, dtype=torch.int64)
    torch.cuda.synchronize()

    tdict['scale'] += time.time() - start_time
    ...
    tdict['Point'] += time.time() - start_time

    start_time = time.time()
    point_dict.serialization(order=order)
    torch.cuda.synchronize()

    tdict['serialization'] += time.time() - start_time
    ...
    tdict['reorder'] += time.time() - start_time
...
class PointMamba(nn.Module):
    ...
    def forward(self, pts, feats, tdict = {}):

        torch.cuda.synchronize()
        start_time = time.time()
        neighborhood, center = self.group_divider(pts, feats, tdict)
        torch.cuda.synchronize()
        tdict['group_divider'] = time.time() - start_time
        ...
        torch.cuda.synchronize()
        tdict['encoder'] = time.time() - start_time
        ...
        torch.cuda.synchronize()
        tdict['pos_embed'] = time.time() - start_time


        # # reordering strategy
        tdict['serialization'] = 0.0
        tdict['Point'] = 0.0
        tdict['misc'] = 0.0
        tdict['reorder'] = 0.0
        tdict['scale'] = 0.0
        ...
        _, _, _, group_input_tokens_backward, pos_backward = serialization_func(center, group_input_tokens, pos,
                                                                                'hilbert-trans', tdict=tdict)
        torch.cuda.synchronize()
        tdict['serialization2'] = time.time() - start_time
        ...
        torch.cuda.synchronize()
        tdict['OrderScale'] = time.time() - start_time
        ...
        torch.cuda.synchronize()
        tdict['blocks'] = time.time() - start_time
